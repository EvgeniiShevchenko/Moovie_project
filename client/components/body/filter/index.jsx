...
const Filter = ({genres}) => {
    ...
    const {filterParametr, setFilterParameter, curentPage, setCurentPage, filterResetButtonHeandler,
        setFilterResetButtonHeandler, searchValue, setSearchValue,} = React.useContext(SearchContext);
    // useState
    ...
    const searchAction = async () => {
        ...
        const searchSetings = {
            ...
            "filter": {
                ...
                "Year": {
                    "from": values.years.fromYear === "" ? 0  : values.years.fromYear === null ? 0 : parseInt(values.years.fromYear),
                    "to": values.years.toYear === "" ? 0  : values.years.toYear === null ? 0 : parseInt(values.years.toYear) 
                },
                "NumOfSeries": {
                    "from": values.countSeries.from === "" ? 0  : values.countSeries.from === null ? 0 : parseInt(values.countSeries.from),
                    "to": values.countSeries.to === "" ? 0  : values.countSeries.to === null ? 0 : parseInt(values.countSeries.to)
                },
        ...
        if(JSON.stringify(searchSetings.filter) === JSON.stringify(filterPervState) && JSON.stringify(searchSetings.search) === JSON.stringify(searchPervState)){
            console.log(searchSetings.filter.status);
            setFilterParameter({...searchSetings, filter: {...searchSetings.filter, status: false}, search: {...searchSetings.search, status: false}});
            
        }else if(JSON.stringify(searchSetings.filter) === JSON.stringify(filterPervState) && JSON.stringify(searchSetings.search) !== JSON.stringify(searchPervState)){
            console.log(searchSetings.filter.status);
            setFilterParameter({...searchSetings, filter: {...searchSetings.filter, status: false}, search: {...searchSetings.search, status: true}});
        }else if(JSON.stringify(searchSetings.filter) !== JSON.stringify(filterPervState) && JSON.stringify(searchSetings.search) === JSON.stringify(searchPervState)){
            console.log(searchSetings.filter.status);
            setFilterParameter({...searchSetings, filter: {...searchSetings.filter, status: true}, search: {...searchSetings.search, status: false}});
        }else{
            console.log(searchSetings.filter.status);
            setFilterParameter({...searchSetings, filter: {...searchSetings.filter, status: true}, search: {...searchSetings.search, status: true}});
        }
    }
    ...
    const listGenres = genre.length !== 0 ? genre.map((genre, index) => {
        return <MenuItem key = {index} value={genre.Name}>{genre.Name}</MenuItem>
              ...
              <Grid item container xs={12} spacing={0} style={{backgroundImage: "url(/static/images/filter-body.png)"}}>
                  ...
                  <Grid item xs={2}>
                      ...
                      <Link href = "/filter" prefetch>
                        <a> <img style = {{width: "26px", marginLeft: "40%", boxShadow: " -2px 2px 3px rgba(0,0,0,0.30)", opacity: "0.85"}} src="/static/images/newdeployWindow.png" alt=""/></a>
                      </Link>
                  ...
                  <Grid style={{margin: "40px 5px 0 5px", backgroundColor: "white", borderRadius: "8px"}} item xs={12}>
                      <FormControl variant="outlined" style={{width: "100%"}}>
                          ...
                          <InputLabel style={{transform: "translate(0, 13px) scale(1)", left: "auto"}} ref={inputLabel}
                              htmlFor="outlined-age-simple">
                              {values.genre.length !== 0 ? values.genre : "По жанру"}
                          </InputLabel>
                          </div>
                          <Select style={{ height: "40px"}} value= "" onChange={(event) => handleChange(event)} input={
                              <OutlinedInput labelWidth={labelWidth} name="genre" id="outlined-age-simple" />
                          }
                          >
                              <MenuItem value="">
                          <em>Пусто</em>
                  ...
                  <Grid style={{margin: "20px 5px 0 5px", backgroundColor: "white", borderRadius: "8px"}} item xs={12}>
                      <FormControl variant="outlined" style={{width: "100%"}}>
                          <InputLabel style={{transform: "translate(95px, 12px) scale(1)"}} ref={inputLabel}
                              htmlFor="outlined-age-simple">
                              {values.type.length !== 0 ? values.type : "По типу"}
                          </InputLabel>
                          <Select style={{ height: "40px"}} value="" onChange={(event) => handleChange(event)} input={
                              <OutlinedInput labelWidth={labelWidth} name="type" id="outlined-age-simple" />
                          }
                          >
                          <MenuItem value="">
                          <em>Пусто</em>
                  ...
                  <Grid style={{margin: "20px 5px 0 5px", backgroundColor: "white", borderRadius: "8px"}} item xs={12}>
                      ...
                      <div style = {{display: "flex", justifyContent: "center"}}>
                          ...
                          <Select style={{ height: "40px"}} value="" onChange={(event) => handleChange(event)}  input={
                              <OutlinedInput labelWidth={labelWidth} name="status" id="outlined-age-simple" />
                          }
                          >
                          <MenuItem value="">
                          <em>Пусто</em>
                  ...
                  <Grid item xs={5} style={{margin: "15px 33px 0 5px", borderRadius: "8px"}}>
                      <TextField id="standard-name" placeholder = "от" value={values.years.fromYear} name = "fromYear"
                          margin="dense" variant="outlined" onChange={(event) => handleChange(event)}
                  ...
                  <Grid item xs={5} style={{margin: "15px 0 0 0", borderRadius: "8px"}}>
                      <TextField id="standard-name" placeholder = "до" margin="dense" value={values.years.toYear} name = "toYear"
                          variant="outlined" onChange={(event) => handleChange(event)}
                  ...
                  <Grid item xs={5} style={{margin: "20px 33px 0 5px", borderRadius: "8px"}}>
                      <TextField id="standard-name" placeholder = "от"  value={values.countSeries.from} onChange={(event) => handleChange(event)}
                          margin="dense" variant="outlined" name = "from"
                          style={{width: "114%", backgroundColor: "white", margin: "0 0 0 0", borderRadius: "8px"}} />
                  </Grid>
                  <Grid item xs={5} style={{margin: "20px 0 0 0", borderRadius: "8px"}}>
                      <TextField id="standard-name" placeholder = "до" margin="dense" onChange={(event) => handleChange(event)}
                          variant="outlined" value={values.countSeries.to} name = "to"
                          style={{width: "113%", backgroundColor: "white", margin: "0 0 0 -13%", borderRadius: "8px"}} />
                  </Grid>
                  <Grid style={{margin: "18px 5px 0 5px", backgroundColor: "white", borderRadius: "8px", zIndex: "0"}} item xs={12}>
                      ...
                      <div style = {{display: "flex", justifyContent: "center", whiteSpace: "nowrap"}}>
                          ...
                          <MenuItem value="R-17+ (насилие и/или нецензурная лексика)">R-17+ (насилие и/или нецензурная лексика)</MenuItem>
                  ...
                  <Grid style={{margin: "18px 5px 0 5px", backgroundColor: "white", borderRadius: "8px", zIndex: "0"}} item xs={12}>
                      ...
                      <div style = {{display: "flex", justifyContent: "center", whiteSpace: "nowrap"}}>
                          ...
                          <Select style={{ height: "40px"}} value="" onChange={(event) => handleChange(event)} onChange={handleChange} input={
                              <OutlinedInput labelWidth={labelWidth} name="sort" id="outlined-age-simple" />
                          }
                          >
                          <MenuItem value="">
                          <em>Пусто</em>
                  ...
                  <Grid container item xs={12} style={{margin: "18px 60px 9px 60px"}} >
                      <Grid item xs={filterParametr.filter.status || filterParametr.search.status ? 9 : 12}>
                        <Button
                            ...
                            variant="contained" onClick = {searchAction}>
                            Искать
                        </Button>
                      </Grid>
                      {filterParametr.filter.status || filterParametr.search.status ? <Grid item xs={3} style = {{zIndex: "10"}}>
                        <input onClick = {filterReset} className = "filterResetButton" type="image" src = "/static/images/newbutton.png"/>
                      </Grid> : null}
                  </Grid>
              </Grid>
          </FilterWraper>
